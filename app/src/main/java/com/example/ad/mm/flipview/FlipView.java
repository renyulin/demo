...
public class FlipView extends FrameLayout {
    ...
    public interface PreOnFlipListener {
        public void PreonFlippedToPage(FlipView v, int position, int offset, long id);
    }
    ...
    private ImgaeLoadCompleteMonitor imgaeLoadCompleteMonitor;
    ...
    private void loadImage() {  // 开始缓存图片
        if (mAdapter != null) {
            for (int i = 0; i < mAdapter.getCount(); i++) {
                flipTo(i);
                if (i >= imgaeLoadCompleteMonitor.imgNum) {
                    break;
                }
            }
            flipTo(0);
            imgaeLoadCompleteMonitor.onComplete();
        }
    }
    ...
    private PreOnFlipListener mPreOnFlipListener;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!mIsFlippingEnabled) {
            Toast.makeText(this.getContext(), toastMsg, Toast.LENGTH_SHORT).show();
            return false;
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mIsFlipping || !mScroller.isFinished() || mPeakAnim != null) {
            ...
            if (degreesFlipped < 90) {
                prePostFlippedToPage(mLastDispatchedPageEventIndex, mScroller.getCurrY());
            }
        } else {
            endScroll();
    ...
    private void postFlippedToPage(final int page) {
        post(new Runnable() {
            ...
            @Override
            public void run() {
                if (mOnFlipListener != null && isUserAction) {
                    isUserAction = false;
    ...
    private void prePostFlippedToPage(final int page, final int offset) {
        post(new Runnable() {

            @Override
            public void run() {
                if (mPreOnFlipListener != null) {
                    mPreOnFlipListener.PreonFlippedToPage(FlipView.this, page, offset,
                            mAdapter.getItemId(page));
                }
            }
        });
     ...
     *                used by the flipview
     */
    public void setAdapter(ListAdapter adapter, ImgaeLoadCompleteMonitor i) {
        imgaeLoadCompleteMonitor = i;
        if (mAdapter != null) {
            mAdapter.unregisterDataSetObserver(dataSetObserver);
        }
    ...
    public void smoothFlipTo(int page) {
        ...
        endFlip();
        int du = getFlipDuration(delta);
        mScroller.startScroll(0, start, 0, delta, du);
        invalidate();
    }
    ...
    public void setPreOnFlipListener(PreOnFlipListener onFlipListener) {
        mPreOnFlipListener = onFlipListener;
    }
    ...
    public boolean ismIsFlippingEnabled() {
        return mIsFlippingEnabled;
    }

    public void setmIsFlippingEnabled(boolean mIsFlippingEnabled) {
        this.mIsFlippingEnabled = mIsFlippingEnabled;
    }
